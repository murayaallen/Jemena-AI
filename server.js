...
app.post('/chat', async (req, res) => {
  try {
    ...
    if (productMatch) {
      ...
      // Start building base response
      let responseText = `🧪 **${productMatch} Information**\n\n`;
      responseText += `**Primary Uses:** ${details.partialUses}\n\n`;

      // Add availability and packaging if available
      if (details.available) {
        responseText += `**Availability:** Currently in stock\n`;
        responseText += `**Packaging Sizes:** ${details.packaging.join(', ')}\n\n`;
      }

      try {
        // Get AI-generated technical details
        ...
        // Add error message while retaining base information
        responseText += `⚠️ Could not retrieve additional details at this time.`;
      }
